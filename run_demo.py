...
# video for test
demo_video = '/content/HQTrack/demo/images'
...
print("SAM init ...")
model_type = 'vit_h'
sam_checkpoint = '/content/HQTrack/segment_anything_hq/pretrained_model/sam_hq_vit_h.pth'
output_mode = "binary_mask"
sam = sam_model_registry[model_type](checkpoint=sam_checkpoint)
sam.to(device=torch.device('cuda'))
...
cfg.TEST_CKPT_PATH = "/content/HQTrack/result/default_InternT_MSDeAOTL_V2/YTB_DAV_VIP/ckpt/save_step_42000.pth"
palette_template = Image.open('/content/HQTrack/my_tools/mask_palette.png').getpalette()
tracker = HQTrack(cfg, config, True, sam_refine,sam_refine_iou)
save_dir = '/content/HQTrack/demo/out'
...
for idx,img_file in enumerate(img_files):
    ...
    if idx == 0:
        ...
        #while (1):
            ...
            #  cv2_imshow('demo', cv2.cvtColor(img4show, cv2.COLOR_RGB2BGR))
            #  if SAM_prompt == 'Box':
            #      OnMouse = OnMouse_box
